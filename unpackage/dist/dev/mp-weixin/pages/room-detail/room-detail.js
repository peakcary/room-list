  ...
  methods: {
    ...
    async loadRoomInfo() {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/room-detail/room-detail.vue:237", "加载房间信息失败:", error);
        common_vendor.index.showToast({
          title: "加载失败",
          icon: "none"
        });
    ...
    async loadMaintenanceRecords() {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/room-detail/room-detail.vue:262", "加载维修记录失败:", error);
      }
    },
    // 显示添加维修弹窗
    showAddMaintenance() {
    ...
    async saveMaintenance() {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/room-detail/room-detail.vue:319", "保存维修记录失败:", error);
        common_vendor.index.showToast({
          title: "保存失败",
          icon: "none"
        });
        ...
        success: async (res) => {
          if (res.confirm) {
            try {
              ...
              common_vendor.index.__f__("error", "at pages/room-detail/room-detail.vue:370", "删除房间失败:", error);
              common_vendor.index.showToast({
                title: "删除失败",
                icon: "none"
              });
    ...
    callTenant(phoneNumber) {
      var _a;
      const phone = phoneNumber || ((_a = this.roomInfo.tenant_info) == null ? void 0 : _a.phone);
      if (phone) {
        common_vendor.index.makePhoneCall({
          phoneNumber: phone,
          fail: (err) => {
            common_vendor.index.__f__("error", "at pages/room-detail/room-detail.vue:392", "拨打电话失败:", err);
            common_vendor.index.showToast({
              title: "拨打失败",
              icon: "none"
            });
    ...
    async loadRentalHistory() {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/room-detail/room-detail.vue:439", "加载出租历史失败:", error);
      }
    },
    // 获取租赁状态文本
