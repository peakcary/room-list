  ...
  methods: {
    ...
    async initDatabase() {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/test/test.vue:142", "数据库初始化失败:", error);
        common_vendor.index.showToast({
          title: "初始化失败",
          icon: "none"
        });
    ...
    async testRoomManagement() {
      ...
      try {
        ...
        this.debugInfo = `Error: ${JSON.stringify(error, null, 2)}`;
        common_vendor.index.__f__("error", "at pages/test/test.vue:176", "房间管理接口测试失败:", error);
      }
    },
    // 添加测试房间
    async addTestRoom() {
      ...
      try {
        ...
        this.debugInfo = `Error: ${JSON.stringify(error, null, 2)}`;
        common_vendor.index.__f__("error", "at pages/test/test.vue:214", "添加测试房间失败:", error);
      }
    },
    // 获取房间列表
    async getRooms() {
      ...
      try {
        ...
        this.debugInfo = `Error: ${JSON.stringify(error, null, 2)}`;
        common_vendor.index.__f__("error", "at pages/test/test.vue:247", "获取房间列表失败:", error);
      }
    },
    // 获取可租用房间
    async getAvailableRooms() {
      ...
      try {
        ...
        this.debugInfo = `Error: ${JSON.stringify(error, null, 2)}`;
        common_vendor.index.__f__("error", "at pages/test/test.vue:281", "获取可租用房间失败:", error);
      }
    },
    // 获取已租用房间
    async getRentedRooms() {
      ...
      try {
        ...
        this.debugInfo = `Error: ${JSON.stringify(error, null, 2)}`;
        common_vendor.index.__f__("error", "at pages/test/test.vue:315", "获取已租用房间失败:", error);
      }
    },
    // 调试数据库状态
    async debugDatabase() {
...
`;
        ...
        this.debugInfo = `Error: ${JSON.stringify(error, null, 2)}`;
        common_vendor.index.__f__("error", "at pages/test/test.vue:375", "调试数据库失败:", error);
      }
    },
    // 修复数据不一致
...
`;
        ...
        this.debugInfo = `Error: ${JSON.stringify(error, null, 2)}`;
        common_vendor.index.__f__("error", "at pages/test/test.vue:431", "修复数据不一致失败:", error);
      }
    },
    // 强制重置数据库
    async forceResetDatabase() {
      ...
      try {
        ...
        this.debugInfo = `Error: ${JSON.stringify(error, null, 2)}`;
        common_vendor.index.__f__("error", "at pages/test/test.vue:475", "强制重置数据库失败:", error);
      }
    },
    // 跳转到系统部署管理页面
    goToSystemDeploy() {
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return {
    ...
    q: common_vendor.o((...args) => $options.goToSystemDeploy && $options.goToSystemDeploy(...args)),
    r: common_vendor.o((...args) => $options.forceResetDatabase && $options.forceResetDatabase(...args)),
    s: common_vendor.t($data.resetResult),
    t: common_vendor.o((...args) => $options.fixDataInconsistencies && $options.fixDataInconsistencies(...args)),
    v: common_vendor.t($data.fixResult),
    w: common_vendor.o((...args) => $options.forceResetDatabase && $options.forceResetDatabase(...args)),
    x: common_vendor.t($data.resetResult),
    y: common_vendor.t($data.debugInfo)
  };
}
